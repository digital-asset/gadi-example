...
import { AuthorizedDap, AccountRequest } from "@daml2js/digital-trust-1.0.0/lib/Main";
...
export default function AccountRequests() {
  ...
  const requests = useStreamQuery(AccountRequest).contracts;

  const isAuthorizedDap = authDaps.length > 0;
  if (!isAuthorizedDap) return (null);

  const handleAccept = async (requestCid : ContractId<AccountRequest>) => {
    await ledger.exercise(AccountRequest.AccountRequest_Accept, requestCid, { authorizedDapCid: authDaps[0].contractId });
  }

  const handleReject = async (requestCid : ContractId<AccountRequest>) => {
    await ledger.exercise(AccountRequest.AccountRequest_Reject, requestCid, {});
  }
